	...
	return (
		<div class="relative h-[600px] min-w-[100vw] overflow-y-hidden">
			<a href={action?.href ?? "#"} aria-label={action?.label}>
				...
				{action && (
					<div
						class="absolute  bottom-[5%] transform-center  max-h-min w-[80%] flex flex-col justify-center items-center gap-4 p-4 rounded sm:(max-w-[50%] top-0 bottom-0 left-0 right-0 fit-content m-auto transform-none) "
						style={{ backdropFilter: "blur(8px)" }}
					>
						<Text variant="title" tone="highlight" class="font-black text-center sm:text-title-desktop">
							{action.title}
						</Text>
						<Text variant="description" tone="highlight" class="sm:text-description-desktop text-center">
							{action.subTitle}
						</Text>
            {
              action.label && (
			...
			<ol class="flex items-center justify-center col-span-full gap-4 z-10 row-start-4">
				{images?.map((_, index) => (
					<li class="h-full">
						...
						>
							<div
								class={tw`group-disabled:${animation(
